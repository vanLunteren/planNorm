int_D <- function (delta = 0, Delta = 4, alpha = 0.05, beta = 0.2, simu = 10000, adj = F,
  ...
  text <- paste("Settings: \n", "\n delta = ", delta, ", Delta = ", Delta, ", alpha = ",
              alpha, ", beta = ", beta, ",", "\n test = ", test, ", nbound = ", nbound, ", nmin = ", nmin)
  ...
  N_pow <- sim_sample_pow(sd = sd_ber, delta = delta, alpha = alpha, beta = beta, Delta = Delta, 
                     prop = prop, simu = simu, test = test, adj = adj, regel= regel, 
  ...
  for (j in 1:3){
    for (i in 1:length(sd_ber)){
      points(sd_ber[i], N_pow[[i]][1, j], col = j)}}
  ...
  for (j in 1:3){
    for (i in 1:length(sd_ber)){
      points(sd_ber[i], N_pow[[i]][2, j], col = j)}}
  abline(h = 0.8, lty = 2)
  abline(h=0.2, lty = 2)
